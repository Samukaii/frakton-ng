...
export class ExampleComponent {
  ...
  onSearch(query: string) {
    // Filter or fetch options based on search query
...
skillActions = [
  ...
  },
  {
    ...
    variant: 'icon',
    identifier: 'edit',
    click: (option) => this.editSkill(option)
  }
];
```
...
@Component({
  template: `
    <fkt-autocomplete
      ...
      [options]="users"
      [loading]="isLoading"
      [noResults]="{label: 'No users found. Try a different search.'}"
      (search)="searchUsers($event)"
    />
  `
})
export class AsyncExampleComponent {
  userControl = new SignalFormControl('');
  users = signal<FktAutocompleteOption[]>([]);
  ...
  async searchUsers(query: string) {
    ...
    try {
      ...
      this.users.set(results.map(user => ({
        value: user.id,
        label: user.name
      })));
    } finally {
...
### FktAutocompleteOption Interface
```typescript
...
- **`options`**: `FktAutocompleteOption[]` - Array of selectable options (default: `[]`)
- **`label`**: `string` - Label text displayed above the input (default: `''`)
- **`placeholder`**: `string` - Placeholder text for input field (default: `''`)
- **`enableAutoCreation`**: `boolean` - Allow creation of new options (default: `false`)
- **`actions`**: `FktButtonAction[]` - Action buttons for each option (default: `[]`)
- **`loading`**: `boolean` - Show loading state in dropdown (default: `false`)
- **`noResults`**: `FktNoResults` - Configuration for no results message

### Output Events
- **`search`**: `EventEmitter<string>` - Emitted when user types in input
...
- **Lazy Loading**: Option to load data only when dropdown opens
