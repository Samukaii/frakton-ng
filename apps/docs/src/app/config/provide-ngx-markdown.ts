import {
    MarkdownModuleConfig,
    MARKED_OPTIONS,
    MarkedOptions,
    MarkedRenderer,
    provideMarkdown,
    SANITIZE,
    SanitizeFunction,
} from 'ngx-markdown';
import { SecurityContext } from '@angular/core';
import { MarkedExtension, Tokens } from 'marked';

export function headingSlug(text: string): string {
    return text
        .toLowerCase()
        .replace(/[^\w\s-]/g, '')
        .replace(/\s+/g, '-');
}
// Função auxiliar para escapar caracteres perigosos no bloco de código
function escapeHtml(text: string): string {
    return text
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&#39;');
}

export const fktMarkdownRenderer: MarkedExtension['renderer'] = {
    heading(token: Tokens.Heading): string {
        // O marked injeta o escopo correto automaticamente no 'this' interno
        const id = headingSlug(token.text);
        const content = this.parser?.parseInline(token.tokens) ?? token.text;
        return `<h${token.depth} id="${id}">${content}</h${token.depth}>\n`;
    },

    code(token: Tokens.Code): string {
        const { lang, text } = token;

        let language = lang || '';
        let fileName = '';

        // Regex para capturar o padrão title="QualquerCoisa" ou title='QualquerCoisa'
        const titleMatch = language.match(/title=["'](.+?)["']/);

        if (titleMatch) {
            fileName = titleMatch[1]; // Captura o nome do arquivo interno das aspas
            // Limpa a string da linguagem para remover o bloco do title
            language = language.replace(/title=["'].+?["']/, '').trim();
        }

        // IMPORTANTE: Escapa o texto do código para não quebrar o HTML
        const escapedText = escapeHtml(text);

        // Monta a estrutura HTML contendo o cabeçalho do arquivo e o código limpo
        return `
            <div class="code-block-container">
              ${
                  fileName
                      ? `<div class="code-block-filename">${fileName}</div>`
                      : ''
              }
              <pre><code class="language-${
                  language || 'plaintext'
              }">${escapedText}</code></pre>
            </div>
            `;
    },
};

const typescriptIcon = `
<svg xmlns="http://www.w3.org/2000/svg"  width="30px" height="30px" viewBox="0 0 256 256" version="1.1" preserveAspectRatio="xMidYMid">
\t<g>
\t\t<polygon fill="#007ACC" transform="translate(128.000000, 128.000000) scale(1, -1) translate(-128.000000, -128.000000) " points="0 128 0 0 128 0 256 0 256 128 256 256 128 256 0 256">

</polygon>
\t\t<path d="M146.658132,223.436863 L146.739401,212.953054 L130.079084,212.953054 L113.418767,212.953054 L113.418767,165.613371 L113.418767,118.273689 L101.63464,118.273689 L89.8505126,118.273689 L89.8505126,165.613371 L89.8505126,212.953054 L73.1901951,212.953054 L56.5298776,212.953054 L56.5298776,223.233689 C56.5298776,228.922577 56.6517824,233.676863 56.8143221,233.798768 C56.9362269,233.961308 77.2130522,234.042577 101.797179,234.001943 L146.536227,233.880038 L146.658132,223.436863 Z" fill="#FFFFFF" transform="translate(101.634640, 176.142993) rotate(-180.000000) translate(-101.634640, -176.142993) ">

</path>
\t\t<path d="M206.566631,234.272145 C213.068219,232.646748 218.025679,229.761668 222.57679,225.048018 C224.933616,222.528653 228.428219,217.936907 228.712663,216.839764 C228.793933,216.514684 217.659965,209.037859 210.914568,204.852462 C210.670758,204.689922 209.69552,205.74643 208.598377,207.371827 C205.306949,212.166748 201.852981,214.239129 196.570441,214.604843 C188.809171,215.133097 183.811076,211.069605 183.851711,204.283573 C183.851711,202.292462 184.136155,201.114049 184.948854,199.488653 C186.65552,195.953414 189.825044,193.840399 199.7806,189.533097 C218.106949,181.649922 225.949489,176.448653 230.825679,169.053097 C236.270758,160.804208 237.489806,147.638494 233.792028,137.845478 C229.728536,127.199129 219.651076,119.966113 205.469489,117.568653 C201.080917,116.796589 190.678377,116.918494 185.964727,117.771827 C175.684092,119.600399 165.931711,124.679764 159.917743,131.343891 C157.560917,133.944526 152.969171,140.730557 153.253616,141.218176 C153.37552,141.380716 154.432028,142.030875 155.610441,142.721668 C156.748219,143.371827 161.05552,145.850557 165.119012,148.207383 L172.473933,152.474049 L174.01806,150.198494 C176.171711,146.907065 180.885362,142.396589 183.729806,140.893097 C191.897425,136.585795 203.112663,137.195319 208.639012,142.15278 C210.995838,144.30643 211.971076,146.541351 211.971076,149.83278 C211.971076,152.799129 211.605362,154.099446 210.061235,156.334367 C208.070123,159.178811 204.006631,161.576272 192.466314,166.574367 C179.259965,172.263256 173.571076,175.798494 168.369806,181.406113 C165.362822,184.656907 162.518377,189.858176 161.339965,194.206113 C160.364727,197.822621 160.120917,206.884208 160.892981,210.541351 C163.61552,223.300716 173.245996,232.199764 187.143139,234.841034 C191.653616,235.694367 202.137425,235.369287 206.566631,234.272145 Z" fill="#FFFFFF" transform="translate(194.578507, 176.190240) scale(1, -1) translate(-194.578507, -176.190240) ">

</path>
\t</g>
</svg>
`;

export function fktMarkdownRendererFactory(): MarkedOptions['renderer'] {
    const renderer = new MarkedRenderer();

    const originalCodeRenderer = renderer.code.bind(renderer);

    renderer.code = function (token) {
        const { lang } = token;
        let language = lang || '';
        let fileName = '';

        const titleMatch = language.match(/title=["'](.+?)["']/);

        if (titleMatch) {
            fileName = titleMatch[1];
            language = language.replace(/title=["'].+?["']/, '').trim();
        }

        const cleanToken = { ...token, lang: language };

        const htmlCode = originalCodeRenderer(cleanToken);

        if (fileName) {
            return `
                <div class="code-block-container">
                    <div class="code-block-files-tab">
                        <div class="code-block-file">
                            ${typescriptIcon}
                            ${fileName}
                        </div>
                    </div>
                  ${htmlCode}
                </div>
            `;
        }

        return htmlCode;
    };

    return renderer;
}

export const withMarkedOptions = (
    options: () => MarkedOptions
): MarkdownModuleConfig => {
    return {
        markedOptions: {
            provide: MARKED_OPTIONS,
            useFactory: options,
        },
    };
};
export const withSanitizer = (
    sanitizer: SecurityContext | SanitizeFunction
): MarkdownModuleConfig => {
    return {
        sanitize: {
            provide: SANITIZE,
            useValue: sanitizer,
        },
    };
};
export const provideNgxMarkdown = (...options: MarkdownModuleConfig[]) => {
    const allOptions = options.reduce(
        (accumulator, current) => ({ ...accumulator, ...current }),
        {}
    );

    return provideMarkdown(allOptions);
};
