import { Component, computed, DOCUMENT, ElementRef, inject, model, output, signal, viewChild } from '@angular/core';
...
export class FktSaturationLightnessSelectorComponent {
    ...
    private onMove = (event: MouseEvent) => {
        ...
        let saturation =
            (Math.min(Math.max(x - barRect.x, minX), maxX) / maxX) * factor;
        let brightness =
            100 - (Math.min(Math.max(y - barRect.y, minY), maxY) / maxY) * 100;
