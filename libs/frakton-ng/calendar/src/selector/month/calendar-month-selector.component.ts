...
export class CalendarMonthSelectorComponent {
    ...
    protected monthButtons = viewChildren('monthButton', { read: ElementRef });
    ...
    protected onGridKeydown(event: KeyboardEvent) {
        ...
        switch (event.key) {
            case 'ArrowLeft':
                nextIndex =
                    this.focusedIndex() % this.colCount === 0
                        ? Math.min(
                              total - 1,
                              this.focusedIndex() + this.colCount - 1
                          )
                        : this.focusedIndex() - 1;
                break;
            case 'ArrowRight':
                nextIndex =
                    this.focusedIndex() % this.colCount === this.colCount - 1 ||
                    this.focusedIndex() === total - 1
                        ? this.focusedIndex() - (this.colCount - 1) >= 0
                            ? this.focusedIndex() - (this.colCount - 1)
                            : 0
                        : this.focusedIndex() + 1;
                if (nextIndex >= total)
                    nextIndex = this.focusedIndex() - (this.colCount - 1);
                break;
            case 'ArrowUp':
                nextIndex = this.focusedIndex() - this.colCount;
                if (nextIndex < 0) {
                    const lastRow =
                        Math.floor((total - 1) / this.colCount) * this.colCount;
