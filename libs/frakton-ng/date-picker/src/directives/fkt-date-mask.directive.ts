...
const formatDate = (value: string) => {
    ...
    chars.forEach((char, index) => {
        ...
        if (index === 7) year = `/${clampYear(year).padStart(4, '0')}`;
    });
...
@Directive({
    selector: 'input[fktDateMask]',
    host: {
        '(input)': 'onInput($any($event.target))',
    },
})
export class FktDateMaskDirective {
    onInput(target: HTMLInputElement) {
        console.log(target);
        const isIso = isIsoDateString(target.value);

        if (isIso) {
            target.value = transformIso(target.value);
            return;
        }
